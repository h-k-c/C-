...
int Devil::findSortByName(string name,string color,int sNum,int taskNum,Task task[],Sort sort[],InfoCons cons_not[],int cons_not_Max,InfoCons cons_notnot[],int cons_notnotMax,Robot robot){
    for(int h=0;h<=sNum;h++){
        for(int k=0;k<=cons_not_Max;k++){
           // cout<<cons_not[k].getNamex()<<cons_not[k].getColorx()<<endl;
            if(sort[h].getsName()==cons_not[k].getNamex()&&sort[h].getsColor()==cons_not[k].getColorx()&&cons_not[k].getColorx()!=" "){
                cout<<"约束物体处理成功"<<endl;
    ...
    for(int y=0;y<=sNum;y++){
        for(int z=0;z<=cons_notnotMax;z++){
            ...
            if(cons_notnot[z].getNamex()==sort[y].getsName()&&cons_notnot[z].getColorx()==sort[y].getsColor()){
                cout<<"bububu"<<endl;
    ...
    if(color==" "&&(name!="plant"||name!="closet"||name!="washmachine"||name!="desk"||name!="table"||name!="microwave"||name!="chair"||name!="human"||name!="couch"||name!="sofa"||name!="worktable"||name!="refigerator"||name!="television"||name!="airconditioner"||name!="refrigerator")){
        //就遍历任务中的物体把有name和color的物体打上标记，防止被占用；
        for(int j=0;j<=taskNum;j++){
            for(int i=0;i<=sNum;i++){
                if(task[j].getTaskNamex()==sort[i].getsName()&&task[j].getTaskColorx()==sort[i].getsColor()&&task[j].getTaskColorx()!=" "){
...
void Devil::CreateAndPut(std::vector<Btree*> &taskTree,Task task[],Sort sort[],Robot robot,int sNum,int taskNum,InfoCons cons_not[],int cons_not_Max,InfoCons cons_notnot[],int cons_notnotMax){
  ...
  //  Btree *b2=new Btree();
    ...
    for(i=0;i<=taskNum;i++){
        if(task[i].getTaskAction()==""||task[i].getTaskAction()=="goto"||task[i].getTaskAction()=="close"){
            continue;
        }
        //Btree *b1;
        //Btree *b2;
    ...
    //b2->next=(Btree *)malloc(sizeof(Btree));
        ...
        for(int m=0;m<=cons_not_Max;m++){
                    if(cons_not[m].getNamex()==sort[state1-1].getsName()){
                        flag=1;
                        break;
                    }
                }
        for(int g=0;g<=cons_notnotMax;g++){
            if(cons_notnot[g].getNamex()==sort[sortNum1-1].getsName()&&cons_notnot[g].getColorx()==sort[sortNum1-1].getsColor()){
                flag=1;
                break;
            }
        }
        ...
        /*{
            ...
            b2->sortNum=sortNum2;
            cout<<"给节点赋值"<<endl;
            if(robot.getHold()==sortNum1||robot.getPlate()==sortNum1){
