...
int Devil::findSortByName(string name,string color,int sNum,int taskNum,Task task[],Sort sort[],InfoCons cons_not[],int cons_not_Max,InfoCons cons_notnot[],int cons_notnotMax,Robot robot){
    for(int h=0;h<=sNum;h++){
        for(int k=0;k<=cons_not_Max;k++){
           // cout<<cons_not[k].getNamex()<<cons_not[k].getColorx()<<endl;
            if(sort[h].getsName()==cons_not[k].getNamex()&&sort[h].getsColor()==cons_not[k].getColorx()&&cons_not[k].getColorx()!=" "&&cons_not[k].getState()!="plate"&&cons_not[k].getState()!="on"){
                cout<<cons_not[k].getNamex()<<cons_not[k].getColorx()<<endl;
                cout<<"约束物体处理成功"<<endl;
                sort[h].setsLockSort(1);
            }
            //如果物体的位置实在约束的物体哪里就加上锁
           /* if(cons_not[k].getNamex()==sort[sort[h].getsLoc()-1].getsName()&&sort[h].getsLoc()!=-1){
                cout<<cons_not[k].getNamex()<<endl;
                cout<<sort[h].getsName()<<endl;
                cout<<sort[sort[h].getsLoc()-1].getsName()<<endl;
                cout<<"处理约束位置"<<endl;
                sort[h].setsLockSort(1);
            }*/
        }
    ...
    for(int y=0;y<=sNum;y++){
        for(int z=0;z<=cons_notnotMax;z++){
            ...
            if(cons_notnot[z].getNamex()==sort[y].getsName()&&cons_notnot[z].getColorx()==sort[y].getsColor()&&cons_notnot[z].getState()!="plate"){
                cout<<"bububu"<<endl;
...
void Devil::CreateAndPut(std::vector<Btree*> &taskTree,Task task[],Sort sort[],Robot &robot,int sNum,int taskNum,InfoCons cons_not[],int cons_not_Max,InfoCons cons_notnot[],int cons_notnotMax){
  ...
  //  Btree *b2=new Btree();
    ...
    cout<<"创建两个节点"<<endl;
    int i;
    if(taskNum<=(cons_notnotMax+cons_not_Max)){
    ...
    for(int v=0;v<=taskNum;v++){
        if(task[v].getTaskAction()=="give"&&task[v].getTaskNamex()==sort[robot.getHold()-1].getsName()){
