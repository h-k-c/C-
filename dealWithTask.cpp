...
void dealWithTask(vector<Btree*> Tasktree,Robot robot,sort sort[]){
 ...
 //首先把机器人手中的物体放下，在不违反约束的情况下；
        ...
        while((*it)!=NULL){
            for(is=(*it)->taskmultimap.begin();is!=(*is)->taskmultimap.end();is++){
                for(isc=is->second.begin();isc!=id->second.end();isc++){
                    ...
                    if(isc->second=="putdown"){
                        ...
                        if(robot.getPlate()!=0&&robot.getHold()!=0){
                            PutDown(robot.getHold());
                            sort[robot.getHold()-1].setLoc(is->first);
                        ...
                        PutDown(isc->first);
                        sort[isc->first-1].setLoc(is->first);
                    }
                    if(isc->second=="putin"){
                        ...
                        sort[isc->first-1].setLoc(is->first);
                        sort[isc->first-1].setInside(is->first);
                    }
